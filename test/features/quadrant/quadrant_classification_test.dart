...
void main() {
  ...
  group('calculateQuadrantMutation tests', () {
    ...
    test('Move Q1 task to Q3 (becomes none priority & preserves urgent endAt)', () {
      final task = createTask(
        id: 'q1',
        priority: TaskPriority.high,
        endAt: overdueMs,
    ...
    test('Move Q2 task (medium priority, future endAt) to Q3 (none priority, endOfToday)', () {
      final task = createTask(
        id: 'q2',
        priority: TaskPriority.medium,
        endAt: futureMs,
    ...
    test('Move Q4 task (none priority, future endAt) to Q2 (high priority, keeps future endAt)', () {
      final task = createTask(
        id: 'q4',
        priority: TaskPriority.none,
        endAt: futureMs,
  ...
  group('QuadrantFilterState tests', () {
    test('matches all tasks when selectedProjectIds is null', () {
      ...
      final t1 = createTask(id: '1', priority: TaskPriority.none, projectId: 'p1');
      final t2 = createTask(id: '2', priority: TaskPriority.none, projectId: 'p2');
    ...
    test('clearProjectIds resets to null', () {
      final state = const QuadrantFilterState(selectedProjectIds: {'p1'})
  ...
  group('buildQuadrantData tests', () {
    test('categorizes and sorts tasks into 4 quadrants', () {
      final t1 = createTask(id: 'q1', priority: TaskPriority.high, endAt: endOfTodayMs);
      final t2 = createTask(id: 'q2', priority: TaskPriority.high, endAt: null);
      final t3 = createTask(id: 'q3', priority: TaskPriority.low, endAt: overdueMs);
    ...
    test('filters tasks by selectedProjectIds', () {
      final t1 = createTask(id: '1', priority: TaskPriority.high, endAt: endOfTodayMs, projectId: 'work');
      final t2 = createTask(id: '2', priority: TaskPriority.high, endAt: endOfTodayMs, projectId: 'personal');
    ...
    test('hides completed tasks by default and shows when showCompleted is true', () {
      final tActive = createTask(id: 'active', priority: TaskPriority.high, endAt: endOfTodayMs);
      final tDone = Task(
        id: 'done',
        projectId: 'inbox',
        parentId: null,
  ...
  group('QuadrantFilterNotifier tests', () {
    test('toggleProject, toggleFolder, resetAll flow', () {
      ...
      expect(container.read(quadrantFilterProvider).selectedProjectIds, isNull);

      final allIds = {'p1', 'p2', 'p3'};
      notifier.toggleProject('p1', allIds);
      // Removed p1 from allIds -> p2, p3 selected
      expect(container.read(quadrantFilterProvider).selectedProjectIds, {'p2', 'p3'});

      notifier.toggleFolder(['p2'], allIds);
      // Removed p2 -> only p3 selected
      expect(container.read(quadrantFilterProvider).selectedProjectIds, {'p3'});

      notifier.toggleFolder(['p1', 'p2'], allIds);
      // Added p1, p2 -> all selected -> resets to null
      expect(container.read(quadrantFilterProvider).selectedProjectIds, isNull);

      notifier.setProjectSelection({'p1'});
      expect(container.read(quadrantFilterProvider).selectedProjectIds, {'p1'});
