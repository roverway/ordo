...
class ScopeNavContent extends ConsumerStatefulWidget {
  ...
  /// 是否作为弹层（Modal BottomSheet / Drawer）渲染。
  /// 若为 true，点击跳转项时会优先调用 Navigator.maybePop() 关闭弹层。
  final bool isModal;
...
class _ScopeNavContentState extends ConsumerState<ScopeNavContent> {
  final Set<String> _collapsedFolders = {};

  void _navigateTo(String path, {bool isPush = false}) {
  ...
  Widget build(BuildContext context) {
    ...
    final currentRoute = widget.currentRoute ?? resolveCurrentRoute(context);
...
/// 支持左滑显露“编辑”与“删除”两个操作按钮的滑动包装组件（桌面端支持右键菜单）。
